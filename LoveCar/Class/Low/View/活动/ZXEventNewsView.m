...
#define ZXEventNewsButtonImageRatio 0.3

@interface ZXEventNewsButton : UIButton
{
    CGFloat imageX;
    CGFloat imageY;
    CGFloat imageW;
...
@implementation ZXEventNewsButton

- (instancetype)init {
    self = [super init];
    if (self) {
        [self.layer setBorderWidth:0.4];
        [self.layer setBorderColor:ZXColor(235, 235, 235).CGColor];
    }
    return self;
}
...
- (CGRect)titleRectForContentRect:(CGRect)contentRect {
    ...
    titleW = contentRect.size.width * (1 - ZXEventNewsButtonImageRatio);
    titleH = imageH;
    titleY = imageY;
...
@interface ZXEventNewsView () <UIActionSheetDelegate>
{
    NSInteger likeCount;
}
...
/** 感兴趣人数 */
@property (nonatomic, weak) ZXEventNewsButton *eventInterestBtn;

/** 活动分享 */
@property (nonatomic, weak) ZXEventNewsButton *eventShareBtn;
...
@implementation ZXEventNewsView
...
- (void)setupEventNewsView {
    ...
    [eventEndTimeLabel setTextColor:ZXColor(136, 137, 145)];
    ...
    [eventPlaceLabel setTextColor:ZXColor(136, 137, 145)];
    eventPlaceLabel.numberOfLines = 0;
    [self addSubview:eventPlaceLabel];
    self.eventPlaceLabel = eventPlaceLabel;
    
    /** 感兴趣人数 */
    ZXEventNewsButton *eventInterestBtn = [[ZXEventNewsButton alloc] init];
    ...
    /** 活动分享 */
    ZXEventNewsButton *eventShareBtn = [[ZXEventNewsButton alloc] init];
...
- (void)setEventNewsCellFrame:(ZXEventNewsCellFrame *)eventNewsCellFrame {
    ...
    ZXEventNews *news = eventNewsCellFrame.eventNews;
 ...
 *  点赞
 */
- (void)eventInterestBtnClicked:(ZXEventNewsButton *)sender event:(UIEvent *)event {
...
- (void)eventShareBtnClicked {
    ...
    ZXShareEventSheet *sheetView = [[ZXShareEventSheet alloc] initWithHeight:0];
